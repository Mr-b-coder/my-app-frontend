...
### ✨ Latest Features (v1.0.81+)

- **🎯 Vertical Alignment Control**: Choose between `'center'` (default) and `'top'` alignment
- **📐 Flexible Sizing**: Custom width, min-width, max-width props for precise control
- **♿ Accessibility**: Full ARIA support, focus trapping, keyboard navigation (Escape to close)
- **🎨 Design System Integration**: Uses design system colors with automatic dark mode support
- **📱 Responsive Design**: Proper viewport constraints and mobile-friendly sizing
- **🔧 Compound Components**: `Modal.Header`, `Modal.Body`, `Modal.Footer` for flexible layouts
...
```tsx
// ✅ Good - small modal for simple confirmations
<Modal size="sm">
  <Modal.Header>Confirm</Modal.Header>
  <Modal.Body>Are you sure?</Modal.Body>
</Modal>

// ✅ Good - larger modal for forms
<Modal size="lg">
...
```tsx
// ✅ Good - Cancel on left, primary action on right
<Modal.Footer>
  <Button variant="ghost" onClick={onClose}>Cancel</Button>
  <Button onClick={onConfirm}>Confirm</Button>
</Modal.Footer>

// ✅ Also acceptable - destructive action warning
<Modal.Footer>
  <Button variant="ghost" onClick={onClose}>Cancel</Button>
...
<Modal 
  isOpen={isOpen} 
  onClose={() => setIsOpen(false)}  // ✅ Must be a function
>
```
...
```tsx
<Button onClick={() => setIsOpen(true)}>  {/* ✅ Button is focusable */}
  Open Modal
</Button>
```
...
**Solution**: The modal uses the design system **modal** layer (`z-50`). The shared scale is: sticky 40, modal 50, sidebar 59/60, toast 9998, overlay 9999 (see **Z-Index Scale** in ACUTRACK_DS_QUICK_REFERENCE and `Z_INDEX` from the package). If your custom elements use a higher z-index, align them to the scale:

```css
/* Sticky headers/footers should use z-40 so modal (z-50) sits above */
.my-sticky-header {
  z-index: 40;
}
```
