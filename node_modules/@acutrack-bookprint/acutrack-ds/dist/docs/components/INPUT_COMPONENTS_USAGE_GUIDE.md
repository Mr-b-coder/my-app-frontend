...
- **Color**: `color?: 'primary' | 'success' | 'warning' | 'danger' | 'info'` — controls border and focus ring color. The unfocused border and focus ring both use the chosen color. Works in both **light and dark mode** (dark mode uses slightly lighter shades for visibility).
...
- **Color prop**: When `color` is set (e.g. `color="danger"`), the input shows a 1px border in that color when unfocused, and the focus ring uses the same color. This applies in both light and dark themes.
- **Error / success / warning**: The `error`, `success`, and `warning` props override the color prop. Error state uses the design system error color and displays correctly in dark mode; success and warning also have dark-mode variants.
- **Precedence**: State styling (error → success → warning) takes precedence over the `color` prop so validation feedback is always visible.
...
- **Keyboard navigation (v1.0.92)**: When the list is open, Arrow Up/Down move the highlighted option, Enter selects, Home/End jump to first/last; Tab/Shift+Tab close and move focus, Escape closes and returns focus to trigger. Focus stays on the trigger so typing works with `allowTyping`. Keyboard-highlighted option uses same style as hover; options with semantic colors (success, warning, error, info) keep their colors when highlighted. Arrow keys do not scroll the page.
...
- **Keyboard navigation (v1.0.92)**: When the list is open, Arrow Up/Down move highlight, Enter toggles option, Home/End jump to first/last; Tab/Shift+Tab close and move focus, Escape closes. Focus stays on trigger (search input or trigger div). Keyboard highlight matches hover and semantic option colors; arrow keys do not scroll the page.
...
- **Focus ring (v1.0.92)**: A single brand focus ring color is used across all input and form components (brand-orange / dark-brand-orange). The **`color`** prop on Input, Textarea, Slider, SearchBar, etc. controls **border** and **focus border** only; the focus ring color is always the brand color for consistency.
...
### 3. Avoiding the browser "Please fill out this field" popup

DS components (Input, PasswordInput, etc.) pass the `required` / `isRequired` prop to the native `<input>` for the required indicator and accessibility. That also triggers the **browser’s native HTML5 validation**: on submit, empty required fields show the browser’s default popup (“Please fill out this field.”).

To use **only your app’s validation** (inline errors, toasts, etc.) and avoid that popup, add **`noValidate`** to the `<form>` and handle validation in your submit handler:
...
In `handleSubmit`, call `e.preventDefault()`, run your validation, set `emailError` / `passwordError`, and optionally show a toast. The browser will no longer show its native validation popup.
