...
- **Keyboard navigation** (Arrow Up/Down, Enter, Home/End, Tab, Shift+Tab, Escape, Backspace) — list highlight and no page scroll (v1.0.92)
...
- **Dropdown overflow protection** (overlay layer z-index: 9999; see **Z-Index Scale** in ACUTRACK_DS_QUICK_REFERENCE)
...
## Keyboard navigation (v1.0.92)

When the dropdown list is open, MultiSelect supports full keyboard navigation aligned with Select and Autocomplete:

- **Arrow Up / Arrow Down**: Move the highlighted option in the list. The highlighted option uses the same visual style as hover (including semantic colors: success, warning, error, info).
- **Enter**: Toggle the currently highlighted option (select if unselected, deselect if selected). In `disableCheckboxes` mode, Enter selects the option.
- **Home / End**: Jump to the first or last option in the list.
- **Tab / Shift+Tab**: Close the dropdown and move focus to the next or previous focusable element (the list does not capture Tab).
- **Escape**: Close the dropdown; when searchable, the input loses focus.
- **Backspace**: When the search input is empty, remove the last selected chip.

Focus remains on the trigger (search input when `searchable`, or the trigger div when not searchable) so you can type to filter while using the arrow keys to move the highlight. Arrow and list-navigation keys do not scroll the page.
