...
- ✅ Multiple theme toggle components for different use cases
...
- Detect the user's system preference on first visit
- Save user's choice to localStorage
- Apply the correct theme immediately on subsequent visits
...
|----------|--------|-------------|
| 1️⃣ | localStorage | User's saved preference from previous visits |
| 2️⃣ | System Preference | OS-level dark/light mode setting |
| 3️⃣ | Default | Falls back to `'light'` mode |
...
### 1. `ThemeToggle` — Standalone Component
...
### 2. `Sidebar.ThemeToggle` — For Sidebars
...
### 3. `Header.ThemeSwitcher` — For Headers
...
- `'default'` — Follows app-level light/dark theme
- `'brand'` — Fixed navy blue theme
- `'minimal'` — Fixed light/white theme
- `'grey'` — Fixed dark grey theme
- `'dark'` — Fixed dark theme
- `'custom'` — Custom colors via `customColors` prop
...
```tsx
// ✅ Correct - provides both variants
...
// ❌ Incorrect - only works in one theme
...
1. **Always provide both variants** — Every color class should have a `dark:` counterpart

2. **Use semantic color tokens** — Use `text-primary` instead of `text-blue-900`

3. **Group theme classes together** — Keep related light/dark classes adjacent for readability

4. **Test both themes** — Always verify your UI looks correct in both light and dark modes

5. **Consider contrast ratios** — Ensure text is readable in both themes (WCAG AA compliance)

6. **Add smooth transitions** — Use `transition-colors duration-200` for smooth theme switches
